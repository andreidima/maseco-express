-- SQL script to mirror the 2025_10_05_182213_create_service_ff_plati_calupuri_fisiere_table migration
...
INSERT INTO `service_ff_plati_calupuri_fisiere` (`plata_calup_id`, `cale`, `nume_original`, `created_at`, `updated_at`)
SELECT `id`, `fisier_pdf`, SUBSTRING_INDEX(`fisier_pdf`, '/', -1), NOW(), NOW()
FROM `service_ff_plati_calupuri`
WHERE `fisier_pdf` IS NOT NULL AND `fisier_pdf` <> '';

-- Optional clean-up of the legacy column once data has been migrated.
ALTER TABLE `service_ff_plati_calupuri`
